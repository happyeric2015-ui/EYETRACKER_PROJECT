"""
Eye-Tracking Experiment: Pro/Antisaccade Task (Enhanced Version)
Designed for Mac embedded camera in a virtual environment
Enhanced with user-friendly features, calibration, and feedback
"""
...
class SaccadeExperimentEnhanced:
    """Enhanced Pro/Antisaccade experiment controller with user-friendly features"""
    ...
    def show_calibration_screen(self, cap):
        """Show calibration screen to verify eye tracking"""
    ...
    def show_calibration_warning(self, cap, quality):
        """Show warning if calibration quality is poor"""
        screen = self.create_blank_screen()
    ...
    def show_instruction_screen(self, cap):
        """Show detailed instruction screen with visual examples"""
        instruction_pages = [
            {
    ...
    def show_countdown(self, cap, message="Get Ready!", duration=3):
        """Show countdown before starting trials"""
        start_time = time.perf_counter()
    ...
    def show_break_screen(self, cap, current_trial, total_trials):
        """Show break screen between trial blocks"""
        screen = self.create_blank_screen()
        ...
        lines = [
            ...
            f"That's {int(current_trial/total_trials*100)}% done!",
            "",
            "Feel free to:",
    ...
    def show_practice_feedback(self, cap, accuracy, rt_ms):
        """Show feedback after practice trial"""
        screen = self.create_blank_screen()
    ...
    def run_trial(self, trial_id, trial_type, target_side, is_practice, total_trials, cap):
        """Run a single trial with enhanced UI"""
        self.gaze_history = []
        rt_ms = None
        accuracy = "Error"
    ...
    def run_experiment(self):
        """Run the complete experiment with enhanced UI"""
        print("Starting Enhanced Eye-Tracking Experiment")
        ...
        if not self.show_countdown(cap, "Starting Practice Trials...", 3):
            cap.release()
            cv2.destroyAllWindows()
            return
        ...
        for trial_id, trial_type, target_side, is_practice in all_trials:
            ...
            if trial_id == self.practice_trials + 1:
                if not self.show_break_screen(cap, self.practice_trials, total_trials):
                    break
                if not self.show_countdown(cap, "Starting Test Trials...", 3):
                    break
            ...
            if not is_practice and trial_id > self.practice_trials and (trial_id - self.practice_trials) % 10 == 0:
                if not self.show_break_screen(cap, trial_id - self.practice_trials, self.test_trials):
                    break
                if not self.show_countdown(cap, "Resuming...", 2):
                    break
    ...
    def show_completion_screen(self, cap):
        """Show experiment completion screen with summary"""
        if not self.results:
            return
...
def main():
    """Main entry point"""
    print("=" * 60)
    print("Pro/Antisaccade Eye-Tracking Experiment (Enhanced)")
    print("=" * 60)
    print("\nPress ENTER to begin...")
    input()

    experiment = SaccadeExperimentEnhanced()
